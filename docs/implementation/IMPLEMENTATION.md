...
:root {
  ...
  --color-accent-pink: #FFB6C1;     /* 粉色 */
  --color-text-brown: #5D4037;      /* 文字褐色 */
...
### 設計原則
1. **所有卡片**：3px粗邊框 + 24px圓角
2. **所有按鈕**：16px圓角 + hover效果
3. **配色方案**：暖色系（黃→橘→紅）
...
| 參考圖片 | 實作檔案 | 主要功能 |
|----------|-------------|---------|
| aminal-01.png | [HomePage.tsx](src/pages/HomePage.tsx) | 分割視圖、PetRoom、ShopGrid |
| aminal-02.png | [ProductDetail.tsx](src/pages/ProductDetail.tsx) | ReactionPreview、購買UI |
| aminal-03.png | [BasketVisualizer.tsx](src/components/Cart/BasketVisualizer.tsx) | 購物籃視覺效果 |

## 🚀 執行方法

### 1. 必要套件
本專案使用 React + TypeScript。
...
### UI 動畫
- `reactionPop`：反應表情符號出現（0.5s）
- `itemDrop`：購物車商品掉落（0.5s）
- `badgePulse`：徽章脈動（2s、無限循環）
- `imageZoomIn`：圖片放大（0.3s）

## 💡 實作要點

### 1. BasketVisualizer 的隨機配置
**問題**：物理引擎太重  
**解決**：使用 CSS Positioning 和 Math.random() 輕量實作
